...
import { MessageCircle, Heart, Send, Trash2, User, X, Smile, ArrowLeft } from 'lucide-react';
...
const GlobalCommentModal: React.FC = () => {
  ...
  useEffect(() => {
    if (!isCommentModalOpen || !currentSuggestionId) return;

    setLoading(true);
    const unsubscribe = subscribeToComments(currentSuggestionId, (newComments) => {
      setComments(newComments);
      setLoading(false);
    });

    return () => unsubscribe();
  }, [currentSuggestionId, isCommentModalOpen]);

  const handleSubmitComment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newComment.trim() || !currentSuggestionId) return;

    setSubmitting(true);
    try {
      const currentUserId = getCurrentUserId();
      await addComment(currentSuggestionId, newComment.trim(), `Usuário ${currentUserId.slice(-4)}`);
      setNewComment('');
    } catch (error) {
  ...
  const handleDeleteComment = async (commentId: string) => {
    if (!confirm('Tem certeza que deseja deletar este comentário?') || !currentSuggestionId) return;

    try {
      await deleteComment(commentId, currentSuggestionId);
    } catch (error) {
  ...
  if (!isCommentModalOpen || !currentSuggestionId) return null;
